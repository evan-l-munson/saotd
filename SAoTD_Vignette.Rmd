...
Once Twitter data has bee acquired the exploration phase can begin by tidying the twitter data. The first tweet in the dataframe is `data$text[1]`, and when it is cleaned and tidy'd it becomes:

```{r}
data("raw_tweets")
data <- raw_tweets
tidy_data <- SAoTD::Tidy(DataFrame = data)
